...
public class VisualScriptingEditor : EditorWindow
{
    ...
    private async UniTask RestoreEditorState()
    {
        if (cacheData == null)
        {
            // 마지막으로 열었던 파일 가져오기
    ...
    private Scenario CreateNewFile()
    {
        // 임시로 사용될 새 파일 생성
        var newFile = CreateInstance<Scenario>();
    ...
    public void OpenFile()
    {
        ...
        // 파일 불러오기
        var currentFile = AssetDatabase.LoadAssetAtPath<Scenario>(path);
    ...
    public void SaveAs()
    {
        ...
        var loadFile = AssetDatabase.LoadAssetAtPath<Scenario>(path);

        // 에셋 데이터 로드에 실패한 경우
        if (loadFile == null)
        {
            // 해당 경로에 에셋 만들기
            loadFile = CreateInstance<Scenario>();
    ...
    private Scenario GetLastOpenedFile()
    {
        ...
        // 해당 파일을 찾아 리턴
        return AssetDatabase.LoadAssetAtPath<Scenario>(path);
    }
    ...
    private void SetCurrentFile(Scenario file)
    {
        // 현재 파일 교체
