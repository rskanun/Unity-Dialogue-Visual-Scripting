...
public class SelectNode : LineNode, ILineProvider
{
    ...
    public Line ToLine()
    {
        return new SelectLine((SelectNodeData)ToData());
    }
    ...
    private List<string> GetOptionKeys()
    {
        return Enumerable.Range(0, choices.Count)
                        .Select(i => $"{VisualScriptingSettings.Instance.SelectOptionKeyPrefix}_{guid}{i}")
                        .ToList();
    }
    ...
    private void AddOption(string option)
    {
        ...
        if (choices.Count >= VisualScriptingSettings.Instance.MaxChoice)
        {
            // 오버되면 만들기 X
            return;
        }
